...
!function (t, n) {
  n.sectionConstructor["dark-press-anchor"] = n.Section.extend({
...
wp.customize.controlConstructor['dark-press-buttonset'] = wp.customize.Control.extend({
  ...
  jQuery(document).ready(function ($) {
    ...
    $('.alpha-color-control').each(function () {
      ...
      colorPickerOptions = {
        ...
        palettes: darkpressColorPalette.colorPalettes // Use the passed in palette.
      ...
      $('<div class="alpha-dark-press-color-picker-container">' + '<div class="min-click-zone click-zone"></div>' + '<div class="max-click-zone click-zone"></div>' + '<div class="alpha-slider"></div>' + '<div class="transparency"></div>' + '</div>').appendTo($container.find('.wp-picker-holder'));
...
wp.customize.controlConstructor['dark-press-dimensions'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
jQuery(document).ready(function ($) {
  ...
  $('.linked').on('click', function () {
    ...
    $this.parent('.link-dark-press-dimensions').removeClass('unlinked');
  }); // Unlinked button
  ...
  $('.unlinked').on('click', function () {
    ...
    $this.parent('.link-dark-press-dimensions').addClass('unlinked');
...
wp.customize.controlConstructor['dark-press-icon'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
jQuery(window).load(function () {
  ...
  jQuery('.customize-control-dark-press-radio-image .buttonset').buttonset(); // Handles setting the new value in the customizer.

  jQuery('.customize-control-dark-press-radio-image input:radio').change(function () {
...
wp.customize.controlConstructor['dark-press-range'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
    ...
    var control = this,
        ...
        input_default,
        changeAction,
        darkpress_range_input_number_timeout; // Update the text value
    ...
    function autocorrect_range_input_number(input_number, timeout) {
      ...
      clearTimeout(darkpress_range_input_number_timeout);
      darkpress_range_input_number_timeout = setTimeout(function () {
        if (isNaN(value)) {
...
wp.customize.controlConstructor['dark-press-slider'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
(function ($, api) {
  api.controlConstructor['dark-press-toggle'] = api.Control.extend({
    ready: function ready() {
      var control = this;
