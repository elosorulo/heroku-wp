 ...
 */
if( !class_exists( 'Dark_Press_Helper' ) ):
	class Dark_Press_Helper{
		...
		public function __construct(){	
			...
			if( self::$object_counter == 0 ){
				...
				# ajax call
				add_action( 'wp_ajax_view_more_posts', array( __CLASS__, 'view_more_posts' ) );
				// add_action( 'wp_ajax_nopriv_view_more_posts', array( __CLASS__, 'view_more_posts' ) );
		...
		public static function skip_content(){
			...
			<a class="skip-link screen-reader-text" href="#content">
				<?php esc_html_e( 'Skip to content', 'dark-press' ); ?>
			</a>
			<?php
		}
		...
		public static function with_prefix( $content, $append = '-', $prepend = '', $imploder = ' ' ){
			if( is_array( $content ) ){
				...
				return esc_html( implode( $imploder, $prefix_cls ) );
			}else{
				return esc_html( $prepend . self::get_prefix() . $append . trim( $content ) );
			}
		}
		...
		public static function fn_prefix( $fn ){
			return esc_html( self::with_prefix( $fn, '_' ) );
		}
		 ...
		 * @package Dark Press WordPress Theme
		 */		
		public static function excerpt_more( $text ){
			if( is_admin() && !wp_doing_ajax() ){
				return $text;
			}
			
			if( '' != dark_press_get( 'read-more-text' ) && dark_press_get( 'post-readmore' ) ){				
				$more = sprintf( '<a href="%1$s" class="read-more">%2$s<i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>',
			        esc_url( get_the_permalink() ),
			        esc_html( dark_press_get( 'read-more-text' ) )
			    );
			}else{
				$more = '';
			}
		 ...
		 * @package Dark Press WordPress Theme
		 */	
		public static function view_more_posts(){
			if ( !wp_verify_nonce( $_POST['nonce'], 'dark-press-nonce' ) ){
			    die;
			}
		...
		*/
		public static function get_class( $cls ){
			if( is_array( $cls ) ){
				...
				return esc_html( implode( ' ', $prefix_cls ) );
			}else{
				return esc_html( self::get_prefix() . '-' . $cls );
			}
		}
		...
		public static function get_comment_number(){
			if( !dark_press_get( 'post-comments' ) ){
				return;
			}
			
			if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				echo '<span class="darkpress-comments">';
				comments_popup_link(
					'<i class="fa fa-comment"></i> '.esc_html__( 'Leave a comment', 'dark-press' ),		
					'<i class="fa fa-comment"></i> '.esc_html__( '1 response', 'dark-press' ),
					'<i class="fa fa-comments"></i> % '. esc_html__( 'responses' , 'dark-press' )
				);
				echo '</span>';
			}
		}		
		...
		*/
		...
		public static function admin_scripts( $hook ){
			if( 'page.php' == $hook || 'post.php' == $hook ){				
				$scripts = array(
				    array(
		...
		*/
		public static function header_style(){
			...
			Dark_Press_Css::add_styles( $style );
		}
		...
		public static function get_meta( $meta_key = false, $post_id = null,  $single = true  ){
			if( $meta_key ){
				...
				return esc_html( get_post_meta( $post_id, $meta_key, $single ) );
			}
		}
		...
		public static function get_theme_path( $file ){
			return esc_html( get_theme_file_path( $file ) );
		}
		...
		*/
		public static function get_assets_version(){
			return esc_html( dark_press_get( 'assets-version' ) );
		}
		    ...
		    switch( $location ){
		        ...
		        case 'primary':
		            ...
		            $menu = wp_nav_menu( array(
		                ...
		                'container_class' =>'darkpress-main-menu'
		            ));
		        break;
		...
		* @package Dark Press WordPress Theme
		*/
		public static function posts_navigation( $total_post = false, $displayed_post = fasle, $echo = true ) {
			...
			if( !$infinity_module_active || !Dark_Press_Helper::is_active_plugin( 'jetpack' ) ){
				if( is_home() && 'custom' == dark_press_get( 'pagination-option' ) ){
					if( $displayed_post < $total_post ){
						echo '<a href="#" class="darkpress-view-more">' . dark_press_get( 'pagination-text' ) . '</a>';
					}					
				}else{
					the_posts_pagination(
						array(
							'mid_size'  => 2,
							'prev_text' => esc_html__( 'Previous', 'dark-press' ),
							'next_text' => esc_html__( 'Next', 'dark-press' ),
						)
					);
				}		
			}
		...
		*/
		public static function post_content_navigation(){
			wp_link_pages( array(
				'before'      => '<div class="page-links">' . esc_html__( 'Pages:', 'dark-press' ),
                'after'       => '</div>',
		...
		*/
		public static function single_post_navigation(){
			the_post_navigation( array(
				'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous Post', 'dark-press' ) . '</span><span class="nav-title">%title</span>',
				'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next Post', 'dark-press' ) . '</span><span class="nav-title">%title</span>',
			));
		}
		...
		* @return url
		*
		* Dark Press WordPress Theme
		*/
		public static function get_day_link(){
			return esc_url( get_day_link( get_the_time('Y'), get_the_time('m'), get_the_time('d') ) );
		}
		...
		public static function the_date( $post_id = null, $status = 'posted' ) {
			...
			if( $show ):
				...
				printf(
					...
					( 'posted' == $status ) ? esc_html__( 'On', 'dark-press' ) : esc_html__( 'Updated on', 'dark-press' ),
					$time_tag
				);
			endif;
		}
				...
				if( !empty( $cat ) ){ ?>
					<ul class="post-categories">
						<?php foreach ( $cat as $c ) { ?>
							<li>
								<?php $color = get_option( "category_$c->term_id" ); 
								if( $color ){
									$hex = $color[ 'category-color' ];
									$style = 'style="background:'. $hex . '"';
								}else{
									$style = '';
								}?>
								<a href="<?php echo esc_url( get_category_link( $c ) ); ?>" <?php echo $style; ?> >
		...
		public static function the_breadcrumb(){
			...
			if( apply_filters( self::fn_prefix( 'show_breadcrumb' ), true ) ): 
			    ...
			    <div id="<?php echo esc_attr( self::with_prefix( 'breadcrumb' ) ) ?>" class="<?php echo esc_attr( $classes ); ?>">
			    	<?php darkpress_breadcrumb_trail( $breadcrumb_args ); ?>
				</div><!-- #breadcrumb -->
			    <?php return;
			endif;
		}
					...
					<?php 
						...
						<?php }elseif(  is_search() ){
							...
							<h2 class="entry-title">
								<?php 
									printf( 
										esc_html__( 'Search Results for: %s', 'dark-press' ), 
		...
		public static function beautify( $string ){
		    return esc_html( ucwords( str_replace( '-', ' ', $string ) ) );
		}
		...
		/**
		* Font family used in Darkmag Theme
		*
		* @static
		* @access public
		* @return object
		...
		*/
		public static function get_font_family( ){
			return apply_filters( self::fn_prefix( 'standard_fonts_array' ), array(
				'font-1'  => esc_html__( 'Lato', 'dark-press' ),
				'font-2'  => esc_html__( 'Oswald', 'dark-press' ),
				'font-3'  => esc_html__( 'Montserrat', 'dark-press' ),
				'font-4'  => esc_html__( 'Roboto', 'dark-press' ),  
				'font-5'  => esc_html__( 'Raleway', 'dark-press' ),  
				'font-6'  => esc_html__( 'Playfair Display', 'dark-press' ),  
				'font-7'  => esc_html__( 'Fjalla One', 'dark-press' ),  
				'font-8'  => esc_html__( 'Alegreya Sans', 'dark-press' ),
				'font-9'  => esc_html__( 'PT Sans Narrow', 'dark-press' ),
				'font-10' => esc_html__( 'Open Sans', 'dark-press' ),
				'font-11' => esc_html__( 'Poppins', 'dark-press' ),
				'font-12' => esc_html__( 'Muli', 'dark-press' ),
			));
		}
		...
		public static function get_google_font(){
			...
			$google_fonts[] = self::uglify( self::get_font( dark_press_get( 'body-font' ) ), $condition );
			$google_fonts[] = self::uglify( self::get_font( dark_press_get( 'heading-font' ) ), $condition );
			$google_fonts[] = self::uglify( self::get_font( dark_press_get( 'site-info-font' ) ), $condition );
			...
			$fonts_weight = apply_filters( self::fn_prefix( 'standard_fonts_weight' ), array(
				...
				'Muli' 		 => array( '200', '300', '400', '500', '600', '700', '800', '900' )
