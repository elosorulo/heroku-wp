...
if( !class_exists( 'Dark_Press_Customizer' ) ){
	class Dark_Press_Customizer extends Dark_Press_Helper{
		...
		*/
		public static function add_custom_control( $control ){
			...
			if( !is_array( $control ) ){
				$err = esc_html__( 'Invalid argument passed for Custom Control. Must be an array with "type" & "class" key.', 'dark-press' );
				self::add_error( $err );
				return;
			}
			...
			if( !isset( $control[ 'type' ] ) ){
				$err = esc_html__( 'Custom Control must contain "type" key ( type of control ).', 'dark-press' );
				self::add_error( $err );
				$flag = false;
			}

			if( !isset( $control[ 'class' ] ) ){
				$err = esc_html__( 'Custom Control must contain "class" key ( name of custom class ).', 'dark-press' );
				self::add_error( $err );
				$flag = false;
			}		

			if( !isset( $control[ 'sanitize' ] ) ){
				$err = esc_html__( 'Custom Control must contain "sanitize" key ( a callback function ).', 'dark-press' );
				self::add_error( $err );
				$flag = false;
			}
		...
		*/
		public static function add_custom_section( $section ){
			...
			if( !is_array( $section ) ){
				$err = esc_html__( 'Invalid argument passed for Custom Section. Must be an array with "type" & "class" key.', 'dark-press' );
				self::add_error( $err );
				return;
			}
			...
			if( !isset( $section[ 'type' ] ) ){
				$err = esc_html__( 'Custom Section must contain "type" key ( type of section ).', 'dark-press' );
				self::add_error( $err );
				$flag = false;
			}

			if( !isset( $section[ 'class' ] ) ){
				$err = esc_html__( 'Custom Section must contain "class" key ( name of section class ).', 'dark-press' );
				self::add_error( $err );
				$flag = false;
			}		
				...
				foreach( $args[ 'fields' ] as $field ){
					...
					if( isset( $field[ 'id' ] ) ){
						...
						if( array_key_exists( $field_id, self::$settings ) ){
							...
							$msg = sprintf( esc_html__( 'Field with ID: %s already exists.', 'dark-press' ), $field[ 'id' ] );
							self::add_error( $msg );
						}
		...
		*/
		private static function validate_argument( $args ){
			...
			if( !is_array( $args ) ){
				...
				$msg = esc_html__( 'You must pass array as an argument in set method.', 'dark-press' );
			...
			if( !isset( $args[ 'section' ] ) ){
				...
				$msg = esc_html__( 'No section found.', 'dark-press' );
				self::add_error( $msg, 2 );
			...
			if( !is_array( $args[ 'section' ] ) ){
				...
				$msg = sprintf( 
					...
					esc_html__( 'Invalid argument', 'dark-press' ),	
					esc_html__( 'section', 'dark-press' ),
					esc_html__( 'Must be a an array with atleast an id, title...', 'dark-press' )
				);
				...
				$msg = esc_html__( 'Section id is missing.', 'dark-press' );
				self::add_error( $msg, 2 );
			}
			...
			if( isset( $args[ 'fields' ] ) && !is_array( $args[ 'fields' ] ) ){
				...
				$msg = sprintf( 
					'%s <strong><em>%s</em></strong>. %s',
					esc_html__( 'Invalid argument', 'dark-press' ),	
					esc_html__( 'fields', 'dark-press' ),
					esc_html__( 'Must be a an array', 'dark-press' )
				);
						...
						<p>
							<?php  
								echo sprintf( '<strong>%s</strong> %s', 
									esc_html__( 'Darkmag Customizer:', 'dark-press' ),
									esc_html( $value ) 
								);  
							?>
						</p>
		...
		*/
		private static function get_panel_arg( $panel ){
			...
			$args = array(
			    ...
			    esc_html__( 'No Title Specified.', 'dark-press' ) : 
			    $panel['title'],
			);
		...
		*/
		private static function get_control_arg( $section, $field, $prefix_on_section = true ){
			...
			if( isset( $field[ 'flex_width' ] ) ){
				$args[ 'flex_width' ] = $field[ 'flex_width' ];
			}

			if( isset( $field[ 'flex_height' ] ) ){
				$args[ 'flex_height' ] = $field[ 'flex_height' ];
			}
		...
		*/
		private static function get_section_arg( $panel_id = false, $section ){
			$args = array(
				'title' => empty( $section[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'dark-press' ) : $section[ 'title' ],
			);
...
add_action( 'customize_register', array( Dark_Press_Customizer::get_instance(), 'register' ) );

if( !function_exists( 'dark_press_get' ) ):
	/**
	 ...
	 * @package Dark Press WordPress Theme
	 */
	function dark_press_get( $id ){
		$id = Dark_Press_Helper::with_prefix( $id );
		return Dark_Press_Customizer::get( $id );
	}
